...
function loadFilePTM(arrayBuffer){
	...
	workerPtm = new Worker('data/js/cunparser/workerPtmLoader.js');
...
function loadFileRTI(arrayBuffer){
	...
	switch(rtiType){
		...
		//loadDataHSH(inputBuffer, w, h, basisTerm, true);//RTI HSH
			workerRti = new Worker('data/js/cunparser/workerRtiLoader.js');
...
function loadFile(arrayBuffer){
	...
	while (true) {
		...
		if(stringTag.s.localeCompare("EOF")==0){
			...
			workerOtherSides = new Worker('data/js/cunparser/workerLoadversion41OtherSides.js');

...
void main(void)
{
  ...
  c = c + (2.0*uParam[0]-1.0)*(c-sum/sumg);
  ...
  vec3 rhoSpecular = vec3(1.0,1.0,1.0)*uParam[1];
  const float sigma = 0.09;
  float cosThetaE = dot(normal,lookDirection);
	vec3 halfDirection = lookDirection + light0;
	float cosGamma = dot(normal,halfDirection);
	float gamma = acos(cosGamma);
	specularTerm += exp(-gamma*gamma/(2.0*sigma*sigma))/cosThetaE;
//  halfDirection = lookDirection + light1;
//	cosGamma = dot(normal,halfDirection);
//	gamma = acos(cosGamma);
//	specularTerm += exp(-gamma*gamma/(2.0*sigma*sigma))/cosThetaE;

  float diffuseTerm0 = max(0.0,dot(normal,light0)); 
  vec3 diffspec0 = c*diffuseTerm0*lightIntensity.x;
  float diffuseTerm1 =  max(0.0,dot(normal,light1)); 
  vec3 diffspec1 = c*diffuseTerm1*lightIntensity.y;

  vec4 total = vec4(lightIntensity.x*diffspec0 + lightIntensity.y*diffspec1, 1.0);//vec4( (diffspec0 + diffspec1 + rhoSpecular*specularTerm) * uParam[0], 1.0);

...
if(uBoolScml == 1.0){
 ...
 // vec3 hsh = hweights0*hsh0 + hweights1*hsh1 + hweights2*hsh2 + hweights3*hsh3;
  ...
  if(uBoolScml == 1.0){
    normal.x = scmlScale[12]*( normal.x - scmlBias[12]);
    normal.y = scmlScale[13]*( normal.y - scmlBias[13]);
    normal.z = scmlScale[14]*( normal.z - scmlBias[14]);
    lightIntensity *= 0.33;
  }
  ...
  nDotH0 = pow(nDotH0, uParam[0]*10.0);
  float spec0 = (hshL0.x + hshL0.y + hshL0.z) * uParam[1] * 2.0/3.0 * nDotH0;
  ...
  nDotH0 = pow(nDotH0, uParam[0]*10.0);
  float spec1 = (hshL1.x + hshL1.y + hshL1.z) * uParam[1] * 2.0/3.0 * nDotH0;
...
//hsh = 0.000005*hweights0*hsh0 + 10000.0*hweights1*hsh1;//+hsh0;
 ...
 //gl_FragColor = vec4(hsh0.x,1.0,1.0,1.0);
	 ...
	 gl_FragColor =  vec4((hshL0 * uParam[2] / 100.0 + spec0) * uLightIntensity0 / 2.0 + (hshL1 * uParam[2] / 100.0 + spec1) * uLightIntensity1 / 2.0, 1.0);
  }
}

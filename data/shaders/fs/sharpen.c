...
int s = int(1.0 + 10.0*uParam1);
  ...
  for (int i=0; i<21; i++){ //21: MAXVALUE of int s = int(1.0 + 10.0*uLightDiffuse1.r);
	for (int y=-1; y<=1; y+=1){ 
		for(int x=-1; x<=1; x+=1){
		  ...
		  // kernel access appears to be very time consuming...(*)
		  //int k = 11*(s-1); // kernel offset entry in the list kernels[144]
		  // (x != 0) g = gkernels[k+abs(x)-1];
		  //else if (y != 0) g = gkernels[k+abs(y)-1];
		  //else if (i == 0) g = 1.0;
		  //else g = 0.0;

		  // (*).. so for now we use linear fall off as compromise
		  g = float(s-i);
  ...
  vec3 total = diffspec0*uLightIntensity0 + diffspec1*uLightIntensity1;
